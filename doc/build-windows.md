...
for Ubuntu Xenial does not produce working executables for some of the NIX Core applications.
...
causes two of the nix executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the nix code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the nix source code).

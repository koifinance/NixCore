...
#include <ghost-address/smsg/db.h>
...
static UniValue smsgoptions(const JSONRPCRequest &request)
{
    ...
    if (mode == "list")
    {
        ...
        if (!request.params[1].isNull()){
            if(request.params[1].isBool())
                fDescriptions = request.params[1].getBool();
            else{
                std::string s = request.params[1].get_str();
                bool rv;
                if (!nix::GetStringBool(s, rv))
    ...
    if (mode == "set")
    {
        ...
        {
            if (nix::GetStringBool(value, fValue))
            {
...
static UniValue smsglocalkeys(const JSONRPCRequest &request)
{
    ...
    if (mode == "recv")
    {
        ...
        bool fValue;
        if (!nix::GetStringBool(op, fValue))
        {
            result.pushKV("result", "Unknown value.");
            return result;
        };
            ...
            || mode == "unread")
        {
            ...
            while (dbInbox.NextSmesg(it, sPrefix, chKey, smsgStored))
            {
                ...
                if (rv == 0)
                {
                    ...
                    if (filter.size() > 0
                        && !(nix::stringsMatchI(msg.sFromAddress, filter, 3) ||
                            nix::stringsMatchI(sAddrTo, filter, 3) ||
                            nix::stringsMatchI(sText, filter, 3)))
                        continue;
    ...
    if (mode == "stats")
    {
        ...
        uint64_t nBytes = 0;
        {
            ...
            for (it = smsgModule.buckets.begin(); it != smsgModule.buckets.end(); ++it)
            {
                ...
                objM.pushKV("last changed", nix::GetTimeString(it->second.timeChanged, cbuf, sizeof(cbuf)));
                ...
                } else
                {
                    try {
                        ...
                        nBytes += nFBytes;
                        objM.pushKV("file size", nix::BytesReadable(nFBytes));
        ...
        objM.pushKV("size", nix::BytesReadable(nBytes));
...
static UniValue smsgview(const JSONRPCRequest &request)
{
    ...
    if (request.params.size() > 0)
    {
        ...
        if (!fMatchAll)
        {
            ...
            } else
            {
                ...
                {
                    if (nix::stringsMatchI(itl->second.name, sTemp, matchType))
                    {
    ...
    while (i < request.params.size())
    {
        ...
        if (sTemp == "-from")
        {
            ...
            i++;
            sTemp = request.params[i].get_str();
            tFrom = nix::strToEpoch(sTemp.c_str());
            if (tFrom < 0)
        ...
        if (sTemp == "-to")
        {
            ...
            i++;
            sTemp = request.params[i].get_str();
            tTo = nix::strToEpoch(sTemp.c_str());
            if (tTo < 0)
    ...
    if (tFrom > 0)
        result.pushKV("from", nix::GetTimeString(tFrom, cbuf, sizeof(cbuf)));
    if (tTo > 0)
        result.pushKV("to", nix::GetTimeString(tTo, cbuf, sizeof(cbuf)));
#else
    UniValue result(UniValue::VOBJ);
    throw JSONRPCError(RPC_MISC_ERROR, "No wallet.");
#endif

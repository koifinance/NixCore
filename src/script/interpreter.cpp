...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, bool isCoinstake, ScriptError* serror)
{
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHashAny())
    {
        ...
        if (flags & SCRIPT_VERIFY_WITNESS) {
            if (pubKey2.IsWitnessProgram(witnessversion, witnessprogram, isCoinstake)) {
                hadWitness = true;
...
size_t CountWitnessSigOps(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, bool isCoinstake)
{
    ...
    if (scriptPubKey.IsPayToScriptHashAny() && scriptSig.IsPushOnly()) {
        ...
        if (subscript.IsWitnessProgram(witnessversion, witnessprogram, isCoinstake)) {
            return WitnessSigOps(witnessversion, witnessprogram, witness ? *witness : witnessEmpty, flags);
        }
    }

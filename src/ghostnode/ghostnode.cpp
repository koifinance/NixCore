...
bool CGhostnode::UpdateFromNewBroadcast(CGhostnodeBroadcast &mnb) {
    ...
    if (fGhostNode && pubKeyGhostnode == activeGhostnode.pubKeyGhostnode) {
        ...
        if (nProtocolVersion == PROTOCOL_VERSION) {
            ...
            LogPrintf("CGhostnode::UpdateFromNewBroadcast -- wrong PROTOCOL_VERSION, re-activate your MN: message nProtocolVersion=%d  PROTOCOL_VERSION=%d\n", nProtocolVersion, PROTOCOL_VERSION);
            return false;
        }
    }
    return true;
...
void CGhostnode::Check(bool fForce) {
    ...
    if (IsPoSeBanned()) {
        ...
        LogPrintf("CGhostnode::Check -- Ghostnode %s is unbanned and back in list now\n", vin.prevout.ToStringShort());
        DecreasePoSeBanScore();
    } else if (nPoSeBanScore >= GHOSTNODE_POSE_BAN_MAX_SCORE) {
        nActiveState = GHOSTNODE_POSE_BAN;
        // ban for the whole payment cycle
        nPoSeBanHeight = nHeight + mnodeman.size();
        LogPrintf("CGhostnode::Check -- Ghostnode %s is banned till block %d now\n", vin.prevout.ToStringShort(), nPoSeBanHeight);
        return;
    }
...
void CGhostnode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack) {
    if (!pindex) {
        LogPrintf("CGhostnode::UpdateLastPaid pindex is NULL\n");
        return;
    }
...
//        LogPrintf("mnpayments.mapGhostnodeBlocks.count(BlockReading->nHeight)=%s\n", mnpayments.mapGhostnodeBlocks.count(BlockReading->nHeight));
//        LogPrintf("mnpayments.mapGhostnodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)=%s\n", mnpayments.mapGhostnodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2));
...
bool CGhostnodeBroadcast::Create(std::string strService, std::string strKeyGhostnode, std::string strTxHash, std::string strOutputIndex, std::string &strErrorRet, CGhostnodeBroadcast &mnbRet, bool fOffline) {
    LogPrintf("CGhostnodeBroadcast::Create\n");
    CTxIn txin;
...
bool CGhostnodeBroadcast::SimpleCheck(int &nDos) {
    ...
    if (!IsValidNetAddr()) {
        LogPrintf("CGhostnodeBroadcast::SimpleCheck -- Invalid addr, rejected: ghostnode=%s  addr=%s\n",
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintf("CGhostnodeBroadcast::SimpleCheck -- Signature rejected, too far into the future: ghostnode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }
    ...
    if (nProtocolVersion < mnpayments.GetMinGhostnodePaymentsProto()) {
        LogPrintf("CGhostnodeBroadcast::SimpleCheck -- ignoring outdated Ghostnode: ghostnode=%s  nProtocolVersion=%d\n", vin.prevout.ToStringShort(), nProtocolVersion);
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrintf("CGhostnodeBroadcast::SimpleCheck -- pubKeyCollateralAddress has the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrintf("CGhostnodeBroadcast::SimpleCheck -- pubKeyGhostnode has the wrong size\n");
        nDos = 100;
        return false;
    }

    if (!vin.scriptSig.empty()) {
        LogPrintf("CGhostnodeBroadcast::SimpleCheck -- Ignore Not Empty ScriptSig %s\n", vin.ToString());
        nDos = 100;
        return false;
    }
...
bool CGhostnodeBroadcast::Update(CGhostnode *pmn, int &nDos) {
    ...
    if (pmn->sigTime > sigTime) {
        LogPrintf("CGhostnodeBroadcast::Update -- Bad sigTime %d (existing broadcast is at %d) for Ghostnode %s %s\n",
    ...
    if (pmn->IsPoSeBanned()) {
        LogPrintf("CGhostnodeBroadcast::Update -- Banned by PoSe, ghostnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }

    // IsVnAssociatedWithPubkey is validated once in CheckOutpoint, after that they just need to match
    if (pmn->pubKeyCollateralAddress != pubKeyCollateralAddress) {
        LogPrintf("CGhostnodeBroadcast::Update -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }

    if (!CheckSignature(nDos)) {
        LogPrintf("CGhostnodeBroadcast::Update -- CheckSignature() failed, ghostnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }
    ...
    if (!pmn->IsBroadcastedWithin(GHOSTNODE_MIN_MNB_SECONDS) || (fGhostNode && pubKeyGhostnode == activeGhostnode.pubKeyGhostnode)) {
        ...
        LogPrintf("CGhostnodeBroadcast::Update -- Got UPDATED Ghostnode entry: addr=%s\n", addr.ToString());
...
bool CGhostnodeBroadcast::CheckOutpoint(int &nDos) {
    ...
    if (!CheckSignature(nDos)) {
        LogPrintf("CGhostnodeBroadcast::CheckOutpoint -- CheckSignature() failed, ghostnode=%s\n", vin.prevout.ToStringShort());
        return false;
    }
    ...
    if (!darkSendSigner.IsVinAssociatedWithPubkey(vin, pubKeyCollateralAddress)) {
        LogPrintf("CGhostnodeMan::CheckOutpoint -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            if (pConfIndex->GetBlockTime() > sigTime) {
                LogPrintf("CGhostnodeBroadcast::CheckOutpoint -- Bad sigTime %d (%d conf block is at %d) for Ghostnode %s %s\n",
...
bool CGhostnodeBroadcast::Sign(CKey &keyCollateralAddress) {
    ...
    if (!darkSendSigner.SignMessage(strMessage, vchSig, keyCollateralAddress)) {
        LogPrintf("CGhostnodeBroadcast::Sign -- SignMessage() failed\n");
        return false;
    }

    if (!darkSendSigner.VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError)) {
        LogPrintf("CGhostnodeBroadcast::Sign -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
bool CGhostnodePing::Sign(CKey &keyGhostnode, CPubKey &pubKeyGhostnode) {
    ...
    if (!darkSendSigner.SignMessage(strMessage, vchSig, keyGhostnode)) {
        LogPrintf("CGhostnodePing::Sign -- SignMessage() failed\n");
        return false;
    }

    if (!darkSendSigner.VerifyMessage(pubKeyGhostnode, vchSig, strMessage, strError)) {
        LogPrintf("CGhostnodePing::Sign -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
bool CGhostnodePing::CheckSignature(CPubKey &pubKeyGhostnode, int &nDos) {
    ...
    if (!darkSendSigner.VerifyMessage(pubKeyGhostnode, vchSig, strMessage, strError)) {
        LogPrintf("CGhostnodePing::CheckSignature -- Got bad Ghostnode ping signature, ghostnode=%s, error: %s\n", vin.prevout.ToStringShort(), strError);
        nDos = 33;
        return false;
    }
    return true;
...
bool CGhostnodePing::SimpleCheck(int &nDos) {
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrintf("CGhostnodePing::SimpleCheck -- Signature rejected, too far into the future, ghostnode=%s\n", vin.prevout.ToStringShort());
        nDos = 1;
        return false;
    }
...
bool CGhostnodePing::CheckAndUpdate(CGhostnode *pmn, bool fFromNewBroadcast, int &nDos) {
...

    {
        ...
        if ((*mi).second && (*mi).second->nHeight < chainActive.Height() - 24) {
            LogPrintf("CGhostnodePing::CheckAndUpdate -- Ghostnode ping is invalid, block hash is too old: ghostnode=%s  blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
            // nDos = 1;
            return false;
        }
    }

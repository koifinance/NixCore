...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight == 1)
        nSubsidy = (38000000 * COIN) + (64 * COIN);
            ...
            if (out->IsType(OUTPUT_STANDARD))
            {
                ...
                if (!pScript->IsUnspendable()) {
                    ...
                    if (!is_spent || txout != coin.out || pindex->nHeight != coin.nHeight || is_coinbase != coin.fCoinBase) {
                        fClean = false; // transaction output mismatch
                        std::cout << "MISMATCH: isspent: " << coin.out.nRounds << " AND " << txout.nRounds;
                    }
                }
            };

            if (!fAddressIndex || (!out->IsType(OUTPUT_STANDARD)))
                continue;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - 0);
...
static int GetWitnessCommitmentIndex(const CBlock& block)
{
    ...
    if (!block.vtx.empty()) {
        for (size_t o = 0; o < block.vtx[0]->vpout.size(); o++) {
            if (block.vtx[0]->vpout[o]->GetPScriptPubKey()->size() >= 38 && (*block.vtx[0]->vpout[o]->GetPScriptPubKey())[0] == OP_RETURN && (*block.vtx[0]->vpout[o]->GetPScriptPubKey())[1] == 0x24 && (*block.vtx[0]->vpout[o]->GetPScriptPubKey())[2] == 0xaa && (*block.vtx[0]->vpout[o]->GetPScriptPubKey())[3] == 0x21 && (*block.vtx[0]->vpout[o]->GetPScriptPubKey())[4] == 0xa9 && (*block.vtx[0]->vpout[o]->GetPScriptPubKey())[5] == 0xed) {
                commitpos = o;
            }
        }
    }

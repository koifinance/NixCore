...
std::string CHDWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageGroup(_("NIX wallet options:"));
...
int CHDWallet::AddStandardInputs(CWalletTx &wtx, CTransactionRecord &rtx,
    ...
    bool sign, CAmount &nFeeRet, const CCoinControl *coinControl, std::string &sError, AvailableCoinsType nCoinType, bool fUseInstantSend)
{
    assert(coinControl);
    nFeeRet = 0;
    CAmount nValue;
...
int CHDWallet::AddStandardInputs(CWalletTx &wtx, CTransactionRecord &rtx,
    std::vector<CTempRecipient> &vecSend, bool sign, CAmount &nFeeRet, const CCoinControl *coinControl, std::string &sError, AvailableCoinsType nCoinType, bool fUseInstantSend)
{
    if (vecSend.size() < 1)
    ...
    if (0 != AddStandardInputs(wtx, rtx, vecSend, sea, pcC, sign, nFeeRet, coinControl, sError, nCoinType))
    {
        // sError will be set
        if (pcC)
...
void CHDWallet::GetScriptForMining(CScript &script)
{
    CPubKey newKey;
    CHDWalletDB wdb(*dbw, "r+");
    if (0 != NewKeyFromAccount(&wdb, idDefaultAccount, newKey, false, false))
    {
        wdb.TxnAbort();
    };
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, AvailableCoinsType nCoinType, bool fUseInstantSend)
{
    ...
    if (0 != AddStandardInputs(wtxNew, rtxTemp, vecSend, sign, nFeeRet, &coin_control, strFailReason, nCoinType))
        return false;
            ...
            for (const auto &txin : tx.vin)
            {
                ...
                while (range.first != range.second)
                {
                    if (range.first->second != tx.GetHash())
                    {
                        ...
                        if (wtxConflicted && wtxConflicted->isAbandoned())
                        {
...
void CHDWallet::AvailableCoins(std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth, AvailableCoinsType nCoinType, bool fUseInstantSend, bool fIncludeImmature) const
{
    vCoins.clear();
    ...
    for (const auto& item : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vpout.size(); i++)
        {
            ...
            bool found = false;
            if (nCoinType == ONLY_DENOMINATED) {
                found = IsDenominatedAmount(wtx.tx->vpout[i]->GetValue());
            } else if (nCoinType == ONLY_NOT40000IFMN) {
                found = !(fGhostNode && wtx.tx->vpout[i]->GetValue() == GHOSTNODE_COIN_REQUIRED * COIN);
            } else if (nCoinType == ONLY_NONDENOMINATED_NOT40000IFMN) {
                if (IsCollateralAmount(wtx.tx->vpout[i]->GetValue())) continue; // do not use collateral amounts
                found = !IsDenominatedAmount(wtx.tx->vpout[i]->GetValue());
                if (found && fGhostNode) found = wtx.tx->vpout[i]->GetValue() != GHOSTNODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
            } else if (nCoinType == ONLY_40000) {
                LogPrintf("nCoinType = ONLY_1000\n");
                LogPrintf("pcoin->vout[i].nValue = %s\n", wtx.tx->vpout[i]->GetValue());
                found = wtx.tx->vpout[i]->GetValue() == GHOSTNODE_COIN_REQUIRED * COIN;
            } else if (nCoinType == ONLY_PRIVATESEND_COLLATERAL) {
                found = IsCollateralAmount(wtx.tx->vpout[i]->GetValue());
            } else {
                found = true;
            }
            if (!found) continue;
...
bool CHDWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl *coinControl, AvailableCoinsType nCoinType, bool fUseInstantSend) const
{
    ...
    if (nCoinType == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(CAmount nDenom, vecPrivateSendDenominations)
        {
            BOOST_FOREACH(const COutput &out, vCoins)
            {
                ...
                if (out.tx->tx->vpout[out.i]->GetValue() == nDenom && nValueRet + nDenom < nTargetValue + nSmallestDenom) {
                    CTxIn txin = CTxIn(out.tx->GetHash(), out.i);
                    int nRounds = GetInputPrivateSendRounds(txin);
                    // make sure it's actually anonymized
                    if (nRounds < nPrivateSendRounds) continue;
...
/**
 *
 * @brief CHDWallet::ZEROCOIN FUNCTIONALITIES
 *
 */

bool CHDWallet::CreateZerocoinMintModel(string &stringError, string denomAmount) {
    ...
    if (pubCoin.validate()) {
        ...
        if (!CHDWalletDB(*dbw).WriteZerocoinEntry(zerocoinTx))
            return false;
        return true;
    } else {
        return false;
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nFeeRet < DEFAULT_TRANSACTION_MINFEE && nChange > 0 && nChange < CENT) {
                    int64_t nMoveToFee = min(nChange, DEFAULT_TRANSACTION_MINFEE - nFeeRet);
                    nChange -= nMoveToFee;
                    nFeeRet += nMoveToFee;
                }
                if (nChange > 0) {
                    ...
                    if (nSubtractFeeFromAmount > 0 && IsDust(newTxOut.get(), ::minRelayTxFee)) {
                        ...
                        for (unsigned int i = 0; i < vecSend.size(); i++) // subtract from first recipient
                        {
                            if (vecSend[i].fSubtractFeeFromAmount) {
                                txNew.vpout[i]->SetValue(txNew.vpout[i]->GetValue() - nDust);
                                if (IsDust(txNew.vpout[i].get(), ::minRelayTxFee)) {
                                    strFailReason = _(
                                            "The transaction amount is too small to send after the fee has been deducted");
                                    return false;
                                }
                ...
                BOOST_FOREACH(const CInputCoin &coin, setCoins)
                    txNew.vin.push_back(CTxIn(coin.walletTX->GetHash(), coin.index, CScript(),
 ...
 * @return
 */
bool CHDWallet::CreateZerocoinSpendTransaction(std::string &toKey, int64_t nValue, libzerocoin::CoinDenomination denomination,
    ...
    CMutableTransaction txNew;
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // Insert change txn at random position:
            txNew.vpout.insert(txNew.vpout.begin() + GetRandInt(txNew.vpout.size() + 1) ,newTxOutStandard);
            ...
            CHDWalletDB(*dbw).ListPubCoin(listPubCoin);
            ...
            CHDWalletDB(*dbw).ListCoinSpendSerial(listCoinSpendSerial);
            BOOST_FOREACH(const CZerocoinSpendEntry &item, listCoinSpendSerial){
                if (spend.getCoinSerialNumber() == item.coinSerial) {
                    ...
                    CHDWalletDB(*dbw).WriteZerocoinEntry(pubCoinTx);
            ...
            if (!CHDWalletDB(*dbw).WriteCoinSpendSerialEntry(entry)) {
            ...
            CHDWalletDB(*dbw).WriteZerocoinEntry(coinToUse);
...
bool CHDWallet::CommitZerocoinSpendTransaction(CWalletTx &wtxNew, CReserveKey &reservekey) {
    {
        LOCK2(cs_main, cs_wallet);
 ...
 * @return
 */
string CHDWallet::MintZerocoin(CScript pubCoin, int64_t nValue, CWalletTx &wtxNew, bool fAskFee) {
 ...
 * @return
 */
string CHDWallet::SpendZerocoin(std::string &toKey, int64_t nValue, libzerocoin::CoinDenomination denomination, CWalletTx &wtxNew,
    ...
    if (!CommitZerocoinSpendTransaction(wtxNew, reservekey)) {
        ...
        entry.hashTx = txHash;
        entry.pubCoin = zcSelectedValue;
        if (!CHDWalletDB(*dbw).EraseCoinSpendSerialEntry(entry)) {
...
void CHDWallet::ListAvailableCoinsMintCoins(vector <COutput> &vCoins, bool fOnlyConfirmed) const {
    vCoins.clear();
    {
        LOCK(cs_wallet);
...
//            LogPrintf("pcoin=%s\n", pcoin->GetHash().ToString());
            ...
            for (unsigned int i = 0; i < pcoin->tx->vpout.size(); i++) {
                ...
                if (pcoin->tx->vpout[i]->GetPScriptPubKey()->IsZerocoinMint()) {
                    ...
                    OUTPUT_PTR<CTxOutStandard> newTxOutStandard = MAKE_OUTPUT<CTxOutStandard>();
                    newTxOutStandard->nValue = pcoin->tx->vpout[i]->GetValue();
                    newTxOutStandard->scriptPubKey = *pcoin->tx->vpout[i]->GetPScriptPubKey();

                    vector<unsigned char> vchZeroMint;
                    vchZeroMint.insert(vchZeroMint.end(), newTxOutStandard->GetPScriptPubKey()->begin() + 6,
                                       newTxOutStandard->GetPScriptPubKey()->begin() + newTxOutStandard->GetPScriptPubKey()->size());
...
/**
 *
 * @brief CHDWallet::GHOSTMODE FUNCTIONALITIES
 *
 */

//unlock wallet and create ghost timer
bool CHDWallet::EnableGhostMode(SecureString strWalletPass, string totalAmount){

    this->NotifyZerocoinChanged.connect(boost::bind(&CHDWallet::NotifyGhostChanged, this, _1, _2, _3, _4, _5));
...
bool CHDWallet::DisableGhostMode(){
    LOCK(this->cs_wallet);
    this->nRelockTime = 0;
    this->NotifyZerocoinChanged.disconnect(boost::bind(&CHDWallet::NotifyGhostChanged, this, _1, _2, _3, _4, _5));
    this->Lock();
    return true;
}

//push mint and spends into a queue
void CHDWallet::NotifyGhostChanged(CHDWallet *wallet, const std::string &pubCoin, int denomination, const std::string &isUsed, ChangeType status)
{
...
bool CHDWallet::SpendAllZerocoins(){

    std::list<CZerocoinEntry> pc;
    CHDWalletDB(*dbw).ListPubCoin(pc);
    CZerocoinState *zerocoinState = CZerocoinState::GetZerocoinState();
    int coinHeight;
...
bool CHDWallet::GhostModeMintTrigger(string totalAmount){
...
bool CHDWallet::GhostModeSpendTrigger(string denomination){
...
/**
 *
 * @brief CHDWallet::GHOSTNODE FUNCTIONALITIES
 *
 */

CAmount CHDWallet::GetAnonymizableBalance(bool fSkipDenominated) const {
...
CAmount CHDWallet::GetAnonymizedBalance() const {
    if (fLiteMode) return 0;
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const {
    ...
    for (unsigned int i = 0; i < tx->vpout.size(); i++) {
        ...
        txOutStandard->nValue = tx->vpout[i]->GetValue();
        txOutStandard->scriptPubKey = *tx->vpout[i]->GetPScriptPubKey();

        const CTxIn txin = CTxIn(hashTx, i);

        if (GetHDWallet(pwallet)->IsSpent(hashTx, i) || !GetHDWallet(pwallet)->IsDenominated(txin)) continue;

//        const int nRounds = pwallet->GetInputPrivateSendRounds(txin);
        const int nRounds = 0;
        if (nRounds >= nPrivateSendRounds) {
            nCredit += GetHDWallet(pwallet)->GetCredit(txOutStandard.get(), ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...
CAmount CHDWallet::GetNeedsToBeAnonymizedBalance(CAmount nMinBalance) const {
    if (fLiteMode) return 0;
...
CAmount CHDWallet::GetDenominatedBalance(bool unconfirmed) const {
    if (fLiteMode) return 0;
...
int CHDWallet::GetRealInputPrivateSendRounds(CTxIn txin, int nRounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        } else if(mDenomWtxes[hash].vpout[nout]->nRounds != -10) {
        ...
        if (IsCollateralAmount(wtx->tx->vpout[nout]->GetValue())) {
            mDenomWtxes[hash].vpout[nout]->nRounds = -3;
        ...
        /*
        BOOST_FOREACH(CTxOut out, wtx->tx->vpout) {
        ...
        */
        ...
        if (!fAllDenoms) {
            mDenomWtxes[hash].vpout[nout]->nRounds = 0;
            LogPrintf("privatesend", "GetRealInputPrivateSendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, mDenomWtxes[hash].vpout[nout]->nRounds);
            return mDenomWtxes[hash].vpout[nout]->nRounds;
        }
        ...
        mDenomWtxes[hash].vpout[nout]->nRounds = fDenomFound
...
int CHDWallet::GetInputPrivateSendRounds(CTxIn txin) const
{
    LOCK(cs_wallet);
...
bool CHDWallet::IsDenominated(const CTxIn &txin) const {
    LOCK(cs_wallet);
    ...
    if (mi != mapWallet.end()) {
        const CWalletTx &prev = (*mi).second;
        if (txin.prevout.n < prev.tx->vpout.size()) {
            return IsDenominatedAmount(prev.tx->vpout[txin.prevout.n]->GetValue());
        }
    }
...
bool CHDWallet::IsDenominatedAmount(CAmount nInputAmount) const {
...
int CHDWallet::CountInputsWithAmount(CAmount nInputAmount) {
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (pcoin->IsTrusted()) {
                ...
                for (unsigned int i = 0; i < pcoin->tx->vpout.size(); i++) {
                    ...
                    if (out.tx->tx->vpout[out.i]->GetValue() != nInputAmount) continue;
                    if (!IsDenominatedAmount(pcoin->tx->vpout[i]->GetValue())) continue;
                    if (IsSpent(out.tx->GetHash(), i) || IsMine(pcoin->tx->vpout[i].get()) != ISMINE_SPENDABLE ||
                        !IsDenominated(txin))
                        continue;
...
bool CHDWallet::HasCollateralInputs(bool fOnlyConfirmed) const {
    vector <COutput> vCoins;
    AvailableCoins(vCoins, fOnlyConfirmed, NULL, 1, MAX_MONEY, MAX_MONEY,0,0, 0x7FFFFFFF , ONLY_PRIVATESEND_COLLATERAL);
    return !vCoins.empty();
}
...
bool CHDWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    vector <COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, 1, MAX_MONEY, MAX_MONEY,0,0, 0x7FFFFFFF , nPrivateSendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT40000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (out.tx->tx->vpout[out.i]->GetValue() < nValueMin / 10) continue;
        //do not allow collaterals to be selected
        if (IsCollateralAmount(out.tx->tx->vpout[out.i]->GetValue())) continue;
        if (fGhostNode && out.tx->tx->vpout[out.i]->GetValue() == GHOSTNODE_COIN_REQUIRED * COIN) continue; //ghostnode input

        if (nValueRet + out.tx->tx->vpout[out.i]->GetValue() <= nValueMax) {
            ...
            txin.prevPubKey = *out.tx->tx->vpout[out.i]->GetPScriptPubKey(); // the inputs PubKey
...
bool CHDWallet::GetCollateralTxIn(CTxIn& txinRet, CAmount& nValueRet) const
{
    vector<COutput> vCoins;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(IsCollateralAmount(out.tx->tx->vpout[out.i]->GetValue()))
        {
            txinRet = CTxIn(out.tx->GetHash(), out.i);
            txinRet.prevPubKey = *out.tx->tx->vpout[out.i]->GetPScriptPubKey(); // the inputs PubKey
            nValueRet = out.tx->tx->vpout[out.i]->GetValue();
            return true;
        }
    }
...
bool CHDWallet::GetGhostnodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    AvailableCoins(vPossibleCoins, true, NULL, 1, MAX_MONEY, MAX_MONEY,0,0, 0x7FFFFFFF , ONLY_40000);
...
bool CHDWallet::GetVinAndKeysFromOutput(COutput out, CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet) {
    // wait for reindex and/or import to finish
    if (fImporting || fReindex) return false;
    ...
    txinRet = CTxIn(out.tx->GetHash(), out.i);
    pubScript = *out.tx->tx->vpout[out.i]->GetPScriptPubKey(); // the inputs PubKey
...
bool CHDWallet::ConvertList(std::vector <CTxIn> vecTxIn, std::vector <CAmount> &vecAmounts) {
    BOOST_FOREACH(CTxIn txin, vecTxIn) {
        if (mapWallet.count(txin.prevout.hash)) {
            CWalletTx &wtx = mapWallet[txin.prevout.hash];
            if (txin.prevout.n < wtx.tx->vpout.size()) {
                vecAmounts.push_back(wtx.tx->vpout[txin.prevout.n]->GetValue());
            }
        } else {
...
bool CHDWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax,
    ...
    AvailableCoins(vCoins, true, NULL, 1, MAX_MONEY, MAX_MONEY,0,0, 0x7FFFFFFF , ONLY_DENOMINATED);
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (nValueRet + out.tx->tx->vpout[out.i]->GetValue() <= nValueMax) {
            ...
            BOOST_FOREACH(int nBit, vecBits) {
                if (out.tx->tx->vpout[out.i]->GetValue() == vecPrivateSendDenominations[nBit]) {
...
bool CHDWallet::CreateCollateralTransaction(CMutableTransaction &txCollateral, std::string &strReason) {
    txCollateral.vin.clear();
    txCollateral.vout.clear();
    txCollateral.vpout.clear();
...
bool CHDWallet::SelectCoinsGrouppedByAddresses(std::vector <CompactTallyItem> &vecTallyRet, bool fSkipDenominated,
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vpout.size(); i++) {
            CTxDestination address;
            if (!ExtractDestination(*wtx.tx->vpout[i]->GetPScriptPubKey(), address)) continue;
            ...
            if (fSkipDenominated && IsDenominatedAmount(wtx.tx->vpout[i]->GetValue())) continue;

            if (fAnonymizable) {
                // ignore collaterals
                if (IsCollateralAmount(wtx.tx->vpout[i]->GetValue())) continue;
                if (fGhostNode && wtx.tx->vpout[i]->GetValue() == GHOSTNODE_COIN_REQUIRED * COIN) continue;
                // ignore outputs that are 10 times smaller then the smallest denomination
                // otherwise they will just lead to higher fee / lower priority
                if (wtx.tx->vpout[i]->GetValue() <= vecPrivateSendDenominations.back() / 10) continue;
                // ignore anonymized
            ...
            item.nAmount += wtx.tx->vpout[i]->GetValue();
...
{
    BOOST_FOREACH(CAmount d, vecPrivateSendDenominations)
        if(tx->tx->vpout[i]->GetValue() == d) return 10000;
    if(tx->tx->vpout[i]->GetValue() < 1*COIN) return 20000;

    //nondenom return largest first
    return -(tx->tx->vpout[i]->GetValue()/COIN);
}

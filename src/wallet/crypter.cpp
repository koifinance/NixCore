...
bool CCryptoKeyStore::Lock()
{
...

    {
        LOCK(cs_KeyStore);
        vMasterKey.clear();
        vpwallets[0]->GetGhostWallet()->Lock();
    }
...
{
    {
        ...
        uint256 hashSeed;
        if (CWalletDB(vpwallets[0]->GetDBHandle()).ReadCurrentSeedHash(hashSeed)) {
            uint256 nSeed;
            if (!GetDeterministicSeed(hashSeed, nSeed)) {
                return error("Failed to read sigma seed from DB. Wallet is probably corrupt.");
            }
            ...
            vpwallets[0]->GetGhostWallet()->SetMasterSeed(seed, true);
            vpwallets[0]->GetGhostWallet()->GenerateMintPool();
        }
    }
    NotifyStatusChanged(this);
    return true;
...
{
    CWalletDB db(vpwallets[0]->GetDBHandle());
    string strErr;
    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteSigmaSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteSigmaSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save sigmaseed to wallet";
    }
...
bool CCryptoKeyStore::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadSigmaSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadSigmaSeed(hashSeed, vchSeed)) {

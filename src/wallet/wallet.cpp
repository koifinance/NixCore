...
bool CWallet::GetFeeForTransaction(const std::vector<CRecipient>& vecSend, CReserveKey& reservekey, CAmount& nFeeRet,
...
bool CWallet::GetSetting(const std::string &setting, UniValue &json)
{
    LOCK(cs_wallet);
...
bool CWallet::SetSetting(const std::string &setting, const UniValue &json)
{
    LOCK(cs_wallet);

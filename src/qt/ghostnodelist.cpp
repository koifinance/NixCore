...
#include <qt/forms/ui_ghostnodelist.h>
...
GhostnodeList::GhostnodeList(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GhostnodeList),
    clientModel(0),
    walletModel(0)
{
    ui->setupUi(this);
...
void GhostnodeList::setClientModel(ClientModel *model)
{
...
void GhostnodeList::setWalletModel(WalletModel *model)
{
    this->walletModel = model;
}

void GhostnodeList::showContextMenu(const QPoint &point)
{
    QTableWidgetItem *item = ui->tableWidgetMyGhostnodes->itemAt(point);
    if(item) contextMenu->exec(QCursor::pos());
}

void GhostnodeList::StartAlias(std::string strAlias)
{
...
void GhostnodeList::StartAll(std::string strCommand)
{
...
void GhostnodeList::updateMyGhostnodeInfo(QString strAlias, QString strAddr, const COutPoint& outpoint)
{
...
void GhostnodeList::updateMyNodeList(bool fForce)
{
...
void GhostnodeList::updateNodeList()
{
...
void GhostnodeList::on_filterLineEdit_textChanged(const QString &strFilterIn)
{
...
void GhostnodeList::on_startButton_clicked()
{
    std::string strAlias;
    {
        LOCK(cs_mymnlist);
...
void GhostnodeList::on_startAllButton_clicked()
{
    // Display message box
    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm all Ghostnodes start"),
...
void GhostnodeList::on_startMissingButton_clicked()
{
...
void GhostnodeList::on_tableWidgetMyGhostnodes_itemSelectionChanged()
{
...
void GhostnodeList::on_UpdateButton_clicked()
{
    updateMyNodeList(true);
}

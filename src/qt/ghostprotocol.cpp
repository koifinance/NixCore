...
#include "ghostprotocol.h"
#include <qt/forms/ui_ghostprotocol.h>
...
GhostProtocol::GhostProtocol(const PlatformStyle *platformStyle, Mode mode, QWidget *parent) :
        QWidget(parent),
        ui(new Ui::GhostProtocol),
        model(0),
        mode(mode){
    ui->setupUi(this);
    ...
    switch (mode) {
        case ForSelection:
            setWindowTitle(tr("Ghost Protocol"));
    ...
    GetHDWallet(vpwallets.front())->ListAvailableCoinsMintCoins(vCoins, true);
...
void GhostProtocol::setModel(AddressTableModel *model) {
    this->model = model;
    if (!model)
        return;
    ...
    proxyModel->setFilterFixedString(AddressTableModel::GhostProtocol);
...
void GhostProtocol::on_ghostNIXButton_clicked() {
...
void GhostProtocol::on_convertGhostButton_clicked() {
...
void GhostProtocol::convertGhostToMeCheckBoxChecked(int state) {
    if (state == Qt::Checked)
    {
        ui->convertGhostToThirdPartyAddress->clear();
        ui->convertGhostToThirdPartyAddress->setEnabled(false);
...
void GhostProtocol::on_exportButton_clicked() {
...
void GhostProtocol::contextualMenu(const QPoint &point) {
...
void GhostProtocol::selectNewAddress(const QModelIndex &parent, int begin, int /*end*/) {

...
The RPM spec file provided here is for NIX-Core 1.0.0 and builds on CentOS
...
reference the release tarball available from https://nix.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[NIX git repository](https://github.com/nix/nix) but are not part of
...
Currently only `Source10` is used, for the example `nix.conf` file.
...
in the nix git repository. At present only an SVG version of the NIX
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nix
...
needed when 0.12.1 is released, a proper fix is already in the NIX git
master branch.
...
idea to build NIX Core against a static version of that library compiled
...
The `nix.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nix-server` package. At this point, most RPM based
...
The `nix-server` package should have SELinux support. How to properly do
...
NIX-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/nix/). He has been
packaging NIX for Fedora at least since 2012.
...
4. I renamed the `nix` package that contains the Qt GUI to `nix-core` as
that appears to be how the general population refers to it, in contrast to
`nix-xt` or `nix-classic`. I wanted to make sure the general population

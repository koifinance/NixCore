...
###   nixcore.org and nix.org and compares them.
...
WORKINGDIR="/tmp/nix_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://nixcore.org"
HOST2="https://nix.org"
BASEDIR="/bin/"
VERSIONPREFIX="nix-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'nix-',
...
if [ $? -ne 0 ]; then
   echo "nix.org failed to provide signature file, but nixcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "nix.org and nixcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the nix Core binary release signing key installed?"
   fi
